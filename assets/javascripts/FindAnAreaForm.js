export default class FindAnAreaForm {
  constructor(searchResult) {
    ...
    // The "Find an area" floating card - collapses to just its header
    // once a search succeeds (the page-level "Search results" summary
    // above the map takes over instead), and can otherwise be
    // expanded/collapsed manually via its own header/close buttons,
    // mirroring the Key panel's own toggle/close pattern
    // (LayerControls.js's setKeyPanelExpanded()).
    this.card = document.getElementById('dl-find-an-area-card')
    this.cardToggle = document.getElementById('dl-find-an-area-card-toggle')
    this.cardClose = document.getElementById('dl-find-an-area-card-close')

    // The toggle always flips whatever the current state is; the close
    // button only ever collapses. Server already renders the correct
    // collapsed/expanded state for first paint (see
    // map-controls-panel/macro.jinja's find_an_area_expanded), so
    // there's no need to force it here on construction.
    if (this.cardToggle) {
      this.cardToggle.addEventListener('click', () => {
        this.setCardExpanded(!!(this.card && this.card.classList.contains('dl-map-overlay-card--collapsed')))
      })
    }
    if (this.cardClose) {
      this.cardClose.addEventListener('click', () => this.setCardExpanded(false))
    }
    ...
    // case (its job is done, the result is shown at page level instead),
    // and without this check we'd immediately re-select it here since the
  ...
  setCardExpanded(expanded) {
    ...
    this.card.classList.toggle('dl-map-overlay-card--collapsed', !expanded)
    if (this.cardToggle) {
      this.cardToggle.setAttribute('aria-expanded', expanded ? 'true' : 'false')
    }
  }
